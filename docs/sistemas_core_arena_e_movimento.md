...
- **Esqueleto e Ongma Epilef:** `uses_dash_action_button()` — **`p*_dash` just pressed** (Shift / B) inicia o dash; direcção: `_dash_dir_sign_from_dash_button()` (movimento horizontal, senão frente do jogador). Duplo toque **não** inicia dash.
